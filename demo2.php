...
use VGrid\Components\Actions\Button; // Class mới
...
use VGrid\Components\Columns\DateTimeColumn;
use VGrid\Components\Columns\ImageColumn;
use VGrid\Components\Columns\BooleanColumn;
use VGrid\Components\Columns\LinkColumn;
use VGrid\Components\Columns\ActionColumn; // Class mới

// Import các Input Component mới
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $form = new DataForm();
    // Định nghĩa lại fields để biết cần lấy dữ liệu nào
...
<?php if ($action === 'list'): ?>
    ...
    <div class="d-flex justify-content-between mb-4">
        <h3>Danh sách Lớp</h3>
        <a href="?action=create" class="btn btn-primary"><i class="bi bi-plus"></i> Thêm mới</a>
    </div>
        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h4 class="mb-0">Thêm Lớp Mới</h4>
                </div>
                <div class="card-body">
                    <?php
                        $form = new DataForm('?action=store');
                        $form->setInputs(getClassFormFields()); // Render form với input xịn
                        echo $form->render();
                    ?>
                </div>
            </div>
